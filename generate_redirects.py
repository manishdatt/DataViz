...
def extract_slug_ipynb(filepath):
    try:
        ...
        # Try notebook-level metadata first
        if "slug" in nb.metadata:
            return nb.metadata["slug"]

        # Fall back: check cell metadata (for legacy/manual cases)
        for cell in nb.cells:
            if "slug" in cell.metadata:
                return cell.metadata["slug"]
    except Exception as e:
        print(f"⚠️ Skipping invalid notebook: {filepath}")
    return None

for root, dirs, files in os.walk(POSTS_DIR):
    # Skip hidden/system dirs like .virtual_documents, .ipynb_checkpoints
    ...
    for file in files:
        if file.endswith((".qmd", ".ipynb")):
            ...
            redirect_file = os.path.join(short_dir, "index.qmd")
